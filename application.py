...
def third():
	...
	def index1():
		if request.method == "POST":
			...
			cur = mysql.connection.cursor()
			cur.execute("SELECT image from image WHERE pathname='{}' and date_taken='{}'".format(pathname1,b_date1))
			mysql.connection.commit()
			result=cur.fetchone()
			print(result)
			a = result
			...
			cur = mysql.connection.cursor()
			cur.execute("SELECT image from image WHERE pathname='{}' and date_taken='{}'".format(pathname1,a_date1))
			mysql.connection.commit()
...
def main():
	...
	def css1():
		...
		a=4.03
		print('Mangrove percentage(Before):', a)
		...
		b=3.93
		print('Mangrove percentage(After):', b)
		#cv2.imshow("mask",mask)
		#cv2.imshow("cam1",img)
